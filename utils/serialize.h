...
*    AUTHOR:cody
*
*    HISTORY:增加version ,version>0时支持新老自动兼容
*
*    DATE:2007-11-23
*
*******************************************************************/

#ifndef _BOSS_SERIALIZE_
#define _BOSS_SERIALIZE_
...
#else
#define _SERIALIZE_(ar,t) do{ boss::serialize::CDummy __o___(#t);ar & __o___ & t ;}while(0);
#endif

#define _SERIALIZE_NAME_(ar,t,name) do{ boss::serialize::CDummy __o___(name);ar & __o___ & t ;}while(0);

#define DUMP_OBJ(obj)    boss::serialize::CSDumper().c_str(obj,#obj,boss::serialize::FOMAT_JSON,false)
#define DUMP_OBJ_ML(obj) boss::serialize::CSDumper().c_str(obj,#obj,boss::serialize::FOMAT_JSON,true)
...
namespace boss
{
    ...
    namespace serialize
    {
        ...
        public:
            ...
            template <class AR>
            AR & serialize(AR & ar){return ar;}

            template <class AR>
          	bool Serialize(AR & bs)
          	{
          		this->serialize(bs);
          		return bs.isGood();
          	}
        ...
        public:
            ...
            inline Buffer & operator &(float f)
			{
				char sz[20] = { 0 };
				int size = snprintf(sz, sizeof(sz), "%f", f);
				std::string sTmp;
				sTmp.assign(sz, size);
				(*this) & sTmp;
				return *this;
			}
			inline Buffer & operator &(double lf)
			{
				char sz[24] = { 0 };
				int size = snprintf(sz, sizeof(sz), "%lf", lf);
				std::string sTmp;
				sTmp.assign(sz, size);
				(*this) & sTmp;
				return *this;
            ...
            template<typename DATA>
            Buffer &operator & (const std::vector<DATA> &vec)
            {
            ...
            inline Buffer &operator & (uint64_t d)
            {
                ...
                {
                    check(boss::serialize::UINT64_HEX_BUF_LEN);
                    char szllBuffer[boss::serialize::UINT64_HEX_BUF_LEN + 1] = {0};

#if defined (__LP64__) || defined (__64BIT__) || defined (_LP64) || (__WORDSIZE == 64)
    #if defined(__APPLE__) && defined(__GNUC__) 
                    snprintf(szllBuffer, boss::serialize::UINT64_HEX_BUF_LEN, "%016llx", d);
    #else
                    snprintf(szllBuffer, boss::serialize::UINT64_HEX_BUF_LEN, "%016lx", d);
    #endif
#else
                    snprintf(szllBuffer, boss::serialize::UINT64_HEX_BUF_LEN, "%016llx", d);
#endif

                    memcpy(_p, szllBuffer, boss::serialize::UINT64_HEX_BUF_LEN);
                    _p += boss::serialize::UINT64_HEX_BUF_LEN;
                }
                return *this;
            }
            ...
            template<typename DATA>
            Buffer &operator & (const std::list<DATA> &ls)
            {
                typedef typename std::list<DATA>::const_iterator MyIterater;
                uint32_t sz = ls.size();
                (*this)&sz;
            ...
            template<typename DATA>
           	Buffer & operator & (const std::set<DATA> & st)
           	{
           		typedef typename std::set<DATA>::const_iterator MyIterater;
           		uint32_t sz = st.size();
           		(*this)&sz;
                   for (MyIterater p = st.begin(); p != st.end() ; ++p)
                   {
           			(*this)&(*p);
                   }
           		return *this;
            ...
            Buffer &operator & (const std::pair<LEFT, RIGHT> &p)
            {
                (*this) & (p.first);
                (*this) & (p.second);
                return *this;
            ...
            Buffer &operator & (const std::map<KEY, VALUE> &mp)
            {
                typedef typename std::map<KEY, VALUE>::const_iterator MyIterater;
                uint32_t sz = mp.size();
                (*this)&sz;
			...
			template<typename KEY,typename VALUE, typename COMPARE_TYPE>
			Buffer & operator & (std::map<KEY,VALUE, COMPARE_TYPE> & mp)
			{
				typedef typename std::map<KEY,VALUE, COMPARE_TYPE>::iterator MyIterater;
				uint32_t sz = mp.size();
				(*this)&sz;
				for (MyIterater p = mp.begin();p != mp.end();++p )
				{
			...
			template<typename KEY,typename VALUE, typename COMPARE_TYPE>
			Buffer & operator & (const std::map<KEY,VALUE, COMPARE_TYPE> & mp)
			{
				typedef typename std::map<KEY,VALUE, COMPARE_TYPE>::const_iterator MyIterater;
				uint32_t sz = mp.size();
				(*this)&sz;
				for (MyIterater p = mp.begin();p != mp.end();++p )
				{
            ...
            template<typename KEY, typename VALUE>
            Buffer &operator & (const std::multimap<KEY, VALUE> &mp)
            {
                typedef typename std::multimap<KEY, VALUE>::const_iterator MyIterater;
                uint32_t sz = mp.size();
                (*this)&sz;
                for (MyIterater p = mp.begin(); p != mp.end(); ++p )
                {
            ...
            template<typename KEY, typename VALUE>
            Buffer &operator & (const __gnu_cxx::hash_map<KEY, VALUE> &mp)
            {
                typedef typename __gnu_cxx::hash_map<KEY, VALUE>::const_iterator MyIterater;
                uint32_t sz = mp.size();
                (*this)&sz;
                for (MyIterater p = mp.begin(); p != mp.end(); ++p )
                {
        ...
        public:
            ...
            inline Parser &operator & (uint64_t &d)
            {
                ...
                else
                {
                    ...
                    char szllBuffer[boss::serialize::UINT64_HEX_BUF_LEN + 1] = {0};
                    memcpy(szllBuffer, _p, boss::serialize::UINT64_HEX_BUF_LEN);
                    _p += boss::serialize::UINT64_HEX_BUF_LEN;
            ...
            inline Parser & operator & (bool & d)
            {
                uint8_t c = 0;
            	check(sizeof(c));
            	memcpy(&c,_p,sizeof(c));
            	_p += sizeof(c);
                d = ((c != 0) ? true : false);
            	return *this;
            }

            inline Parser & operator &(float & f)
			{
				std::string sTmp;
				(*this) & sTmp;
				std::stringstream ss;
			...
			inline Parser & operator &(double & f)
			{
				std::string sTmp;
				(*this) & sTmp;
				std::stringstream ss;
            ...
            template<typename DATA>
            Parser & operator & (std::set<DATA> & st)
            {
            	st.clear();
            	uint32_t sz = 0;
            	DATA t;
			...
			template<typename KEY,typename VALUE,typename COMPARE_TYPE>
			Parser & operator & (std::map<KEY, VALUE, COMPARE_TYPE> & mp)
			{
				mp.clear();
				uint32_t sz = 0;
				(*this)&sz;
				for ( uint32_t i = 0;i<sz;++i )
				{
					...
					(*this)&key;
					(*this)&value;
					mp.insert(typename std::map<KEY, VALUE, COMPARE_TYPE>::value_type(key,value));
				}
				return *this;
			}
        ...
        class NilParser
        	...
        	public:
        		template<typename DATA>
        		NilParser & operator &(DATA & d)
        		{
        			d.serialize(*this);
        			return *this;
        		}
        		inline NilParser & operator & (CDummy & d)
        		{
        			(void)d;
        			return *this;
        		}
        		inline NilParser & operator & (int32_t & d)
        		{
        			d = 0;
        			return *this;
        		}
        		inline NilParser & operator & (std::string & s)
        		{
        			s.clear();
        			return *this;
        		}
        		inline NilParser & operator & (uint32_t & d)
        		{
        			d = 0;
        			return *this;
        		}
        		inline NilParser & operator & (uint64_t & d)
        		{
        			d = 0;
        			return *this;
        		}
        		inline NilParser & operator & (int64_t & d)
        		{
        			d = 0;
        			return *this;
        		}
        		inline NilParser & operator & (int16_t & d)
        		{
        			d = 0;
        			return *this;
        		}
        		inline NilParser & operator & (uint16_t & d)
        		{
        			d = 0;
        			return *this;
        		}
        		inline NilParser & operator & (int8_t & d)
        		{
        			d = 0;
        			return *this;
        		}
        		inline NilParser & operator & (uint8_t & d)
        		{
        			d = 0;
        			return *this;
        		}
        		...
        		template<typename DATA>
        		NilParser & operator & (std::list<DATA> & ls)
        		{
        			ls.clear();
        			return *this;
        		}
        		...
        		template<typename KEY,typename VALUE>
        		NilParser & operator & (std::map<KEY,VALUE> & mp)
        		{
        			mp.clear();
        			return *this;
        		}
        		...
        		template<typename KEY,typename VALUE>
        		NilParser & operator & (std::multimap<KEY,VALUE> & mp)
        		{
        			mp.clear();
        			return *this;
        		}
        		template<typename DATA>
        		NilParser & operator & (std::vector<DATA> & vec)
        		{
        			vec.clear();
        			return *this;
        		}
...
#define DUMP(T)\
    ...
    {\
        ...
        return *this;\
            ...
            CDumper &operator & (const std::string &d)
            {
                if (!m_sPostfix.empty())
                {
                    m_ss << m_sPostfix;
            ...
            {
                return (*this) & ((const std::string &)d);
            }
            ...
            template<typename DATA>
            CDumper & operator & (std::set<DATA> &st)
            {
                SeriaContainerHead();
                typename std::set<DATA>::iterator it = st.begin();
                uint32_t dwPos = 0;
                char szId[16] = {0};
                for ( ; it != st.end(); ++it,++dwPos ) 
                {
                    snprintf(szId,sizeof(szId),"%u",dwPos);
                    CDummy o(szId);
                    (*this) << o << *it;
